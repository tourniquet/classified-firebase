...
export default function Item (): JSX.Element {
  ...
  // TODO: These are temporary used for refactoring purpose only
  const title = 'some title here'
  const zoomedImage = false
  const category = 'Imobiliare'
  const subcategory = 'Apartamente'
  const user_id = 2
  const visitor_name = 'John'
  const price = 15
  const currency = '£'
  const views = 104
  const description = 'some shitty description here'
  const region = 'Woolwich, London'
  const phone = '0744 944 9271'

  return (
    <Wrapper>
      <div className='item-page'>
        <Head>
          <title>{title}</title> {/* TODO: get title from props */}
        </Head>
        ...
        <div className='item'>
          ...
          <div className='item-details'>
            ...
            {/* <div className='thumbnails'>
              {images.map(el => (
                <div className='desktop-version thumbnail'>
                  <Image
                    key={el}
